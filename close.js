...
// ── Piano ──────────────────────────────────────────────────────────────────
function pianoVoice(){const k=currentKey,pool=k.scale.filter(n=>n>=48&&n<=76),pPool=k.penta.filter(n=>n>=50&&n<=72);if(!pool.length||!pPool.length)return[60];const r=Math.random();
  // single note
  if(r<.15)return[pPool[0|Math.random()*pPool.length]];
  // open 5th (scale-checked)
  if(r<.33){const root=pool[0|Math.random()*Math.min(8,pool.length)];const fifth=root+7;return(pool.includes(fifth)?[root,fifth]:[root]).filter(n=>n<=84);}
  // two-octave open fifth: root + fifth + fifth-above-octave
  if(r<.47){const lo=pool.filter(n=>n<=60);if(!lo.length)return[pool[0]];const root=lo[0|Math.random()*Math.min(6,lo.length)];const fifth=root+7;if(!pool.includes(fifth))return[root];const notes=[root,fifth];if(fifth+12<=84&&pool.includes(fifth+12))notes.push(fifth+12);return notes;}
  // bass + treble only (hollow two-octave gap)
  if(r<.57){const bp=pool.filter(n=>n<=55),tp=pool.filter(n=>n>=67);if(!bp.length||!tp.length)return[pool[0|Math.random()*pool.length]];return[bp[0|Math.random()*bp.length],tp[0|Math.random()*tp.length]];}
  // minor triad (scale steps)
  if(r<.79){const di=0|Math.random()*Math.max(1,pool.length-4);return[pool[di],pool[Math.min(di+2,pool.length-1)],pool[Math.min(di+4,pool.length-1)]].filter(Boolean);}
  // wide spread
  const di=0|Math.random()*Math.max(1,pool.length-6);return[pool[di],(pool[di+2]||pool[di+1]),(pool[di+4]||pool[di+3]),(pool[di+6]||pool[di+5])].filter(Boolean);}
const PIANO_FX=[
...
const PIANO_PHRASES=[
  ...
  {name:'High, falling close',dur:24,build(out){const r=mkIR(11,2.4);r.connect(out);const d=AC.createGain();d.gain.value=.22;d.connect(out);return seq(transpose([{midi:72,time:0,vel:.70,rel:5},{midi:69,time:5,vel:.74,rel:2},{midi:67,time:5.4,vel:.72,rel:4},{midi:70,time:9.5,vel:.68,rel:2},{midi:72,time:9.9,vel:.72,rel:5},{midi:65,time:14,vel:.76,rel:4},{midi:57,time:18.5,vel:.84,rel:20}]),r,{extra:d});}},
  {name:'Bass-treble dialogue',dur:26,build(out){const r=mkIR(11,2.4);r.connect(out);const d=AC.createGain();d.gain.value=.22;d.connect(out);return seq(transpose([{midi:50,time:0,vel:.82,rel:7},{midi:74,time:5,vel:.68,rel:2},{midi:72,time:5.4,vel:.70,rel:4},{midi:53,time:10,vel:.78,rel:6},{midi:69,time:14.5,vel:.66,rel:2},{midi:67,time:14.9,vel:.68,rel:2},{midi:72,time:15.3,vel:.72,rel:5},{midi:50,time:20,vel:.84,rel:20}]),r,{extra:d});}},
  {name:'Wide descent',dur:46,build(out){const r=mkIR(16,2.7);r.connect(out);return seq(transpose([{midi:74,time:0,vel:.74,rel:14},{midi:72,time:5,vel:.70,rel:13},{midi:69,time:10.5,vel:.78,rel:13},{midi:65,time:15,vel:.74,rel:12},{midi:62,time:21,vel:.76,rel:12},{midi:58,time:26,vel:.72,rel:13},{midi:55,time:30.5,vel:.70,rel:13},{midi:50,time:36,vel:.84,rel:18}]),r);}},
];
function schedulePiano(){
  ...
  if(piLoaded&&!active.piano&&Math.random()<.72){
    ...
    else{const k=currentKey,cr=Math.random();
    if(cr<.12){const r=mkIR(1.2+Math.random()*.6,1.5);r.connect(out);const echoIn=mkEcho(r,.2+Math.random()*.35,.28+Math.random()*.18);const dryG=AC.createGain();dryG.gain.value=.45;dryG.connect(out);hit(pianoVoice(),echoIn,{v:v*1.3,rel:7,spread:sp,extra:dryG});setLog('piano · echo · '+k.name);dur=14;}
    else if(cr<.30){const fx=PIANO_FX[0|Math.random()*PIANO_FX.length];fx.fn(out,pianoVoice(),v,sp);setLog('piano · '+fx.name+' · '+k.name);dur=10;}
    else if(cr<.48){const pool=k.scale.filter(n=>n>=50&&n<=74);const total=5+(0|Math.random()*3);const si=0|Math.random()*Math.max(1,pool.length-total*2);const all=[];for(let i=0;i<total;i++){const idx=Math.min(si+i*2,pool.length-1);if(!all.includes(pool[idx]))all.push(pool[idx]);}const ic=Math.min(3,all.length-2),init=all.slice(0,ic),trail=all.slice(ic);const r=mkIR(1.5+Math.random()*.5,1.6);r.connect(out);const d=AC.createGain();d.gain.value=.40;d.connect(out);hit(init,r,{v:v*1.2,rel:7,spread:sp,extra:d});let cd=0;const bg=.4+Math.random()*.4;trail.forEach((midi,i)=>{cd+=bg*Math.pow(1.35,i);const tv=Math.max(.35,velJ(v*(1.1-i*.07),.05));setTimeout(()=>{if(playing)hitBuf(pianoBufs,[midi],r,{v:tv,rel:7,atk:.04,extra:d});},cd*1000);});setLog('piano · trailing · '+k.name);dur=20+trail.length*2;}
    else if(cr<.63){const fx=PIANO_FX[0|Math.random()*PIANO_FX.length];fx.fn(out,pianoVoice(),v,sp);const rp=k.penta.filter(n=>n>=62&&n<=76);if(rp.length>=2){setTimeout(()=>{if(!playing)return;const rr=mkIR(9+Math.random()*3,2.3);rr.connect(out);const dr=AC.createGain();dr.gain.value=.24;dr.connect(out);const ri=0|Math.random()*Math.max(1,rp.length-2),rt=Math.random();let ev;if(rt<.33)ev=[{midi:rp[ri],time:0,vel:.60,rel:3},{midi:rp[Math.min(ri+1,rp.length-1)],time:.4,vel:.56,rel:16}];else if(rt<.66)ev=[{midi:rp[Math.min(ri+1,rp.length-1)],time:0,vel:.64,rel:16}];else ev=[{midi:rp[Math.min(ri+2,rp.length-1)],time:0,vel:.58,rel:2},{midi:rp[Math.min(ri+1,rp.length-1)],time:.4,vel:.54,rel:2},{midi:rp[ri],time:.8,vel:.64,rel:16}];seqBuf(pianoBufs,ev,rr,{extra:dr});},(4+Math.random()*2)*1000);}setLog('piano · call+response · '+k.name);dur=22;}
    else if(cr<.78){const fx=PIANO_FX[0|Math.random()*PIANO_FX.length];fx.fn(out,pianoVoice(),v,sp);const pp=k.penta.filter(n=>n>=57&&n<=74),lp=k.penta.filter(n=>n>=48&&n<=62);if(pp.length>=2&&lp.length){setTimeout(()=>{if(!playing)return;const rr=mkIR(11,2.4);rr.connect(out);const dr=AC.createGain();dr.gain.value=.22;dr.connect(out);const ri=0|Math.random()*Math.max(1,pp.length-1),land=lp[0|Math.random()*Math.min(4,lp.length)];let ev;if(Math.random()<.5)ev=[{midi:pp[ri],time:0,vel:.62,rel:3},{midi:pp[Math.max(0,ri-1)],time:.4,vel:.58,rel:5},{midi:land,time:5.5,vel:.78,rel:18}];else ev=[{midi:pp[Math.min(ri+1,pp.length-1)],time:0,vel:.62,rel:3},{midi:pp[ri],time:.7,vel:.58,rel:2},{midi:pp[Math.max(0,ri-1)],time:1.1,vel:.64,rel:4},{midi:land,time:6.5,vel:.78,rel:18}];seqBuf(pianoBufs,ev,rr,{extra:dr});},(4+Math.random()*2)*1000);}setLog('piano · resolve · '+k.name);dur=28;}
    else if(cr<.92){const midis=pianoVoice();const fx=PIANO_FX[0|Math.random()*PIANO_FX.length];fx.fn(out,midis,v,sp);const top=midis[midis.length-1],shift=(Math.random()<.4?24:12)*(Math.random()<.5?1:-1);const echo=Math.max(28,Math.min(84,top+shift));setTimeout(()=>{if(!playing)return;const rr=mkIR(12+Math.random()*3,2.5);rr.connect(out);hitBuf(pianoBufs,[echo],rr,{v:velJ(.38,.05),rel:14,atk:shift>0?.10:.06});},(4+Math.random()*2)*1000);setLog('piano · echo-oct · '+k.name);dur=22;}
    else{const fx=PIANO_FX[0|Math.random()*PIANO_FX.length];fx.fn(out,pianoVoice(),v,sp);const hp=k.scale.filter(n=>n>=65&&n<=76),lp=k.penta.filter(n=>n>=50&&n<=64);if(hp.length>=3&&lp.length){setTimeout(()=>{if(!playing)return;const rr=mkIR(11,2.4);rr.connect(out);const dr=AC.createGain();dr.gain.value=.22;dr.connect(out);const dc=3+(0|Math.random()*2),dn=[];for(let i=0;i<dc;i++)dn.push(hp[Math.max(0,hp.length-1-i)]);const times=[0,1.0,2.2,3.8],land=lp[0|Math.random()*Math.min(4,lp.length)];const ev=dn.map((m,i)=>({midi:m,time:times[i]!==undefined?times[i]:times[times.length-1]+(i-times.length+1)*1.5,vel:Math.max(.38,.62-i*.06),rel:i===dc-1?5:3}));ev.push({midi:land,time:times[Math.min(dc-1,times.length-1)]+5,vel:.78,rel:18});seqBuf(pianoBufs,ev,rr,{extra:dr});},(4+Math.random()*2)*1000);}setLog('piano · descent · '+k.name);dur=28;}}
